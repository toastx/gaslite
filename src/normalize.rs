...
//!
//! - [`lexical_key`] — comments stripped, whitespace collapsed, **identifiers and literals kept**.
//!   Used as the result-cache key so two submissions that differ only in formatting/comments
//!   collapse to one entry. Safe to return the cached *output* for, because the optimized code
//!   never echoes the input's formatting.
//!
//! - [`structural`] — additionally maps every user identifier to `ID`, numbers to `NUM`, and
//!   string/hex/address literals to placeholders, while keeping EVM/ Solidity builtins (`msg`,
//!   `sender`, `sload`, `keccak256`, …) as anchors. This is a *structural fingerprint*:
//!   α-equivalent code (same shape, different names) normalizes identically. Used only for
//!   **detection/matching** ([`PatternMatcher`]), never to reuse output — matching is safe, output
//!   reuse across renames is not.
