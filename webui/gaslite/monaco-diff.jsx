/* Gaslite — Monaco-backed diff editor.
   Replaces the hand-built DiffTable: a real side-by-side diff editor with an
   editable original pane, a custom "gaslite" light theme matching the app
   palette, a minimal Solidity Monarch grammar, native collapsing of unchanged
   regions, and the hover-to-explain optimizations rebuilt as line-anchored
   decorations + a custom popover. Exported as window.MonacoDiff. */
const { useState, useEffect, useRef } = React;

/* ---- one-time language + theme registration ---- */
let __glMonacoReady = false;
function glSetupMonaco(monaco) {
  if (__glMonacoReady) return;
  __glMonacoReady = true;
...
function MonacoDiff({ phase }) {
  const hostRef = useRef(null);
  const refs = useRef({});
  const [ready, setReady] = useState(false);
  const [why, setWhy] = useState(null);   // { reason, top }
  ...
  useEffect(() => {
    let dead = false;
    window.glLoadMonaco().then((monaco) => {
      if (dead || !hostRef.current) return;
      glSetupMonaco(monaco);
      const original = monaco.editor.createModel(window.GL.ORIGINAL_SRC, "sol");
      const modified = monaco.editor.createModel(window.GL.ORIGINAL_SRC, "sol");
      ...
      const kick = function () { diff.layout(); oe0.layout(); me0.layout(); oe0.render(true); me0.render(true); };
      requestAnimationFrame(kick);
      [60, 200, 500, 1000].forEach(function (t) { setTimeout(kick, t); });
      ...
      const updateHover = (pos) => {
        if (phaseRef.current !== "done" || !pos) { setWhy(null); return; }
        const reason = refs.current.whyByLine[pos.lineNumber];
        if (!reason) { setWhy(null); return; }
      ...
      me.onMouseMove((e) => updateHover(e.target && e.target.position));
  ...
  useEffect(() => {
    ...
    setWhy(null);
    if (r.deco) { r.deco.clear(); r.deco = null; }
    if (r.flash) { r.flash.clear(); r.flash = null; }
    if (r.flashT) { clearTimeout(r.flashT); r.flashT = 0; }

    const repaint = function () { diff.layout(); me.render(true); r.oe.render(true); };

    if (phase === "done") {
      modified.setValue(window.GL.OPTIMIZED_SRC);
      const whyByLine = {}, decos = [], flashes = [];
      window.GL.OPTIMIZATIONS.forEach((o) => {
        ...
        decos.push({ range: new monaco.Range(line, 1, line, 1), options: { isWholeLine: true, linesDecorationsClassName: "gl-why-bar", className: "gl-why-line" } });
      ...
      r.flashT = setTimeout(() => { if (r.flash) { r.flash.clear(); r.flash = null; } }, 1200);
    } else {
      r.whyByLine = {};
      modified.setValue(r.original.getValue());
    }
    requestAnimationFrame(repaint);
    [60, 220, 500].forEach(function (t) { setTimeout(repaint, t); });
  }, [phase, ready]);

  const w = why && window.GL.REASONS[why.reason];
  return (
    <div className="mc-wrap">
      ...
      {w && (
        <div className="why-pop" style={{ top: why.top, right: 22, left: "auto", pointerEvents: "none" }}>
          ...
          <div className="why-gas"><span>Saves</span><b>{w.gas}</b></div>
        </div>
      )}
    </div>
  );
